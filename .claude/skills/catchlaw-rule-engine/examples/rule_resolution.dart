...
// ruleset TAGGED not filtered, a disagreeing tie returned as Ambiguous instead of chosen, the fixed
// FindingKind precedence, and the three distinct "nothing applies" states. Every date is injected.
...
typedef Request = ({String speciesId, String jurisdiction, WaterType waterType,
  List<String> zonePath, DateTime on, List<Citation> searched, DateTime checkedOn,
  Measurement? reading});
...
class RuleRow {
  const RuleRow(this.speciesId, this.zoneId, this.scope, this.kind, this.lineageId, this.citation,
      this.validFrom, {this.jurisdiction = 'AE', this.waterType = WaterType.marine,
      this.validTo, this.threshold, this.unit, this.method});
  final String speciesId, lineageId, jurisdiction;
...
Resolution resolve(Request req, List<RuleRow> rows) {
  ...
  final applicable = rows.where((r) => r.jurisdiction == req.jurisdiction &&
      r.speciesId == req.speciesId && r.waterType == req.waterType &&
  ...
  final ranked = latest.values.where((r) => r.zoneId == null || req.zonePath.contains(r.zoneId))
      .toList()..sort((a, b) => b.scope.specificity.compareTo(a.scope.specificity));
  ...
  // Expiry does NOT break the tie; nor does recency, strictness or row order.
  if (rivals.length > 1 && rivals.any((r) => !r.outcomeEquals(top))) return Ambiguous(rivals);
...
Finding _finding(RuleRow r, DateTime on, Measurement? read) {
  ...
  // The statement is a fact: no "keep", no "return", no "throw it back".
  final statement = fails && r.kind == FindingKind.minSize && read != null
...
void main() {
  ...
  switch (resolve(req, [hamourMinSize])) {
    ...
    case MethodMismatch(:final read, :final wanted):
      print('Measured as ${read.label}; the rule is ${wanted.label}. Not compared.');
  }
}
