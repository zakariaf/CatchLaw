...
# glyph backwards only in the locale nobody screenshots. Heuristic greps, not a compiler — passing
# this is a floor, not proof the ruler is accurate. The ONLY escape hatch is a trailing
...
#   3. Directionality or TextDirection.of read inside a *_painter.dart
...
report "length held as double/String (store int lengthMm)" \
  "$({ grep -rnE '(double|String)[[:space:]]+[a-zA-Z_]*([Ll]ength|[Ss]ize|[Ww]idth)[a-zA-Z_]*[[:space:]]*[;=,)]' --include='*.dart' "$TARGET" || true
       grep -rnE '[a-zA-Z_]*[Ll]ength(Cm|Metres|Meters|Inches)\b' --include='*.dart' "$TARGET" || true
       grep -rnE '(RealColumn|TextColumn)[[:space:]]+get[[:space:]]+[a-zA-Z_]*([Ll]ength|[Ss]ize)' --include='*.dart' "$TARGET" || true; } \
...
# 3. A CustomPainter has no BuildContext; textDirection is a constructor field.
report "direction read inside a painter (pass labelDirection in, compare in shouldRepaint)" \
  "$(grep -rnE 'Directionality|TextDirection\.of\(|BuildContext' --include='*_painter.dart' "$TARGET" \
     | grep -vE "$GEN_LINE_RE" | grep -vE 'final TextDirection|required this\.|TextDirection\.(ltr|rtl)' | grep -v "$HATCH_RE" || true)"
